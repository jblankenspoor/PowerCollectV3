 ...
 * 
 * @module App
 * @version 3.4.6 - Fixed TypeScript error in PowerFXImportDialog
 */
...
const AppContent: React.FC = () => {
  ...
  const handleGeneratePowerFX = async () => {
    ...
    try {
      ...
      let errorMessage = '// Error generating Power FX code.\n';
      ...
      } else if (String(error).includes('API key')) {
        errorMessage += '// API key error: Please set your Claude API key in the .env file.\n';
      } else if (String(error).includes('429')) {
        errorMessage += '// Rate limit exceeded: Too many requests to the Claude API.\n';
      } else if (String(error).includes('403')) {
        errorMessage += '// Authentication error: Your Claude API key may be invalid.\n';
      }
      
      errorMessage += '// Please check the console for more details.';
      
      // Set error message as code
      dispatch({ 
      ...
      // Reset loading state
      dispatch({ type: 'SET_GENERATING_POWERFX', payload: false });
    }
  };
